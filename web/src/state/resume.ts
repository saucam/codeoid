 ...
 *
 * Plain module state (not a Solid store): cursors are read at attach time,
 * never rendered.
 */
...
export function noteReplayFrame(msg: ScrollbackReplayMsg): void {
  ...
  const existing = cursors.get(msg.sessionId);
  if (existing && existing.key === msg.resumeKey) {
    if (msg.maxSeq > existing.seq) existing.seq = msg.maxSeq;
  } else {
    cursors.set(msg.sessionId, { key: msg.resumeKey, seq: msg.maxSeq });
  }
}
