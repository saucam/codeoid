 ...
 * failed because the saved key is invalid / expired. Supports two paths:
 *
 *  1. ZeroID API key — trades the key for a daemon JWT via /oauth2/token.
 *  2. OAuth (Google or local dev) — PKCE redirect through /auth/authorize,
 *     completed by the /auth/callback page which stores the JWT in
 *     localStorage and redirects back to /ui/.
 *
...
const SignIn: Component<{ onSignedIn: () => void }> = (props) => {
  ...
  const [oauthProvider, setOauthProvider] = createSignal<"google" | "local" | null>(null);
  ...
  function signInWithOAuth(): void {
    if (busy()) return;
    setBusy(true);
    setRegisterError(null);
    void startOAuthLogin().catch((err) => {
      setRegisterError(err instanceof Error ? err.message : String(err));
      setBusy(false);
    });
    // On success, startOAuthLogin redirects the page — no cleanup needed.
  }
  ...
  return (
    <div class="flex h-full items-center justify-center bg-bg p-8">
      ...
      >
        ...
        {/* OAuth sign-in buttons — shown when the daemon has OAuth configured */}
        <Show when={oauthProvider()}>
          {(provider) => (
            <div class="space-y-3">
              <button
                type="button"
                ...
                disabled={busy()}
                class="flex w-full items-center justify-center gap-2 rounded border border-border bg-bg px-3 py-2 text-sm font-medium text-fg transition hover:border-accent/40 hover:bg-bg-hover disabled:cursor-not-allowed disabled:opacity-50"
              >
                <Show when={provider() === "google"} fallback={<span>Sign in (dev mode)</span>}>
                  <GoogleIcon />
