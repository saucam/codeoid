...
(`MemoryEngine.searchSessions`, `Xenova/bge-small-en-v1.5` hybrid). Reproduce:
...
|---|---|---|---|---|---|
| **Within-workspace** (you already know the repo) | 89.2% | 0.937 | 100% | 100% | 487 / 1276 ms |
| **Cross-workspace** (conductor's real need — naive per-ws merge) | **21.6%** | 0.452 | 73.0% | 81.1% | 1994 / 4543 ms |
...
- **Cross-workspace it collapses to 21.6% P@1** — but **R@5 is 81%**, so the right
  session is usually in the top 5; the *ranking* is what's broken.
- **Failure mode:** ~22 of the 29 misses return the SAME wrong session at #1
...
3. **Native cross-workspace mode** (P1 1d) — one global search, embed the query
   once, instead of 11 per-workspace searches — which also fixes the 2–4.5 s
   latency (naive merge) back under the sub-2 s budget.

**Target:** lift cross-workspace P@1 from **21.6%** toward the within-workspace
